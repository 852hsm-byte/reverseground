...
### CSS
- **모든 색상·폰트 크기·border-radius·spacing·border-width는 raw 값(hex/px/rem) 직접 사용 금지 — 반드시 `tokens.css`의 `var(--*)`만 사용한다.** (2026-06 전체 코드베이스 토큰화 완료 — `0`, `50%`처럼 구조적 키워드는 예외)
- **토큰 3단계 구조**: `--size-*`(숫자 원시값) → `--spacing-*` / `--radius-*` / `--font-size-*` / `--border-width-*`(시맨틱) → 컴포넌트 CSS. 새 px 값이 필요하면 `--size-*`에 먼저 추가하고 시맨틱 토큰에서 참조할 것
...
- 웜 뉴트럴 보더·배경(`--sand-50`~`--sand-950`, 밝은→어두운 순): 카드 테두리·탭 배경 등에서 쓰는 베이지 계열 — `neutral-*`(쿨톤 그레이)와 다른 별도 스케일이니 섞어 쓰지 말 것
...
- border-radius 확장: `--radius-3/13/14/20/24` (기존 sm/md/lg/xl/2xl/3xl/rounded 사이·바깥 값)
- font-size 확장: `--font-size-10/11/12/13/15/16/18/22/24/26/28/30/32/34/38/42/deco` — body/heading 스케일에 없는 컴포넌트 전용 크기 (모두 `--size-*` 참조, rem 토큰 삭제됨)
- 임의 색상 추가 금지 — 반드시 `tokens.css` 팔레트 내에서 사용
- **`rgba(...)` 투명도 오버레이(호버/보더/배지 등)는 토큰화 대상에서 의도적으로 제외함.** 183곳·고유 64개 조합이라 양이 크고, 제대로 하려면 색상 저장 방식을 hex 문자열에서 RGB triplet으로 바꿔야 해서(`rgba(var(--brand-rgb), 0.1)` 형태) 색상/radius/font-size 토큰화보다 구조 변경이 더 큼. 대부분 컴포넌트별 1회성 미세조정이라 실익도 낮다고 판단해 보류 — raw `rgba(...)` 그대로 둬도 가이드 위반 아님
...
- 실제 탭 컨테이너: bg `var(--sand-600)` / border-radius `var(--radius-14)` / padding `var(--spacing-6)`
- 실제 활성 탭: bg `var(--white)` / box-shadow만, border-top 없음
- 실제 패널(desktop): border 없음, bg transparent, padding `var(--spacing-64) 0 var(--spacing-40)` / mobile: `var(--spacing-28) 0 var(--spacing-36)`
